...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr', type=float, default=0.001, help='目标学习率')
    ...
    parser.add_argument('--first_threshold', type=float, default=0.95, 
                      help='第一个准确率阈值，达到此值后降低学习率')
    parser.add_argument('--second_threshold', type=float, default=0.99, 
    ...
    save_dir = f'runs/source_domain_{args.model}_{args.backbone}_{args.distance}_{timestamp}'
    ...
    for epoch in range(start_epoch, training_params['n_epochs']):
        ...
        # 当验证准确率达到第一个阈值时，降低学习率
        if not lr_reduced and val_acc >= training_params['first_threshold'] and scheduler.get_last_lr()[0] > 1e-5:
            # 降低学习率
            ...
            if training_params['scheduler'] == 'cosine':
                main_scheduler = optim.lr_scheduler.CosineAnnealingLR(
                    optimizer,
                    T_max=training_params['T_max'] - epoch,
                    eta_min=training_params['eta_min']
                )
        ...
        # 当验证准确率达到第二个阈值时，停止训练
        if val_acc >= training_params['second_threshold']:
            print(f"验证准确率达到{val_acc:.4f}，达到目标阈值，停止训练")
            break
